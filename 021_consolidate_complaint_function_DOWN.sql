-- ============================================================================
-- 021 DOWN: Rollback to 010 and 011 overloads
-- ============================================================================

-- Recreate 010 overload (TEXT)
...
BEGIN
  ...
  VALUES (
    ...
    v_photos_jsonb, v_urls_jsonb, p_location, p_latitude, p_longitude,
  ...
  RETURN jsonb_build_object('success', true, 'complaint_id', v_complaint_id, 'ticket_id', v_ticket_id);
EXCEPTION
  WHEN OTHERS THEN
...
-- Recreate 011 overload (11 params)
CREATE OR REPLACE FUNCTION public.svc_create_complaint(
  p_title            TEXT,
  p_description      TEXT,
  p_category         TEXT,
...
BEGIN
  RETURN public.svc_create_complaint(
    p_title, p_description, p_category, p_location, p_latitude, p_longitude,
    p_photos, p_photo_urls, NULL, p_is_anonymous, p_anonymous_phone, p_village_id
  );
END;
$$;
